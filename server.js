...
// In-memory video token store: token -> { fid, username, ip, expires }
const videoTokens = new Map();
// In-memory watch stats: fid -> { seconds: {[second]: [{username, ts}]}, heatmap: [count per second] }
...
app.get("/f/:fid", (req, res) => {
  ...
  if (vtoken && videoTokens.has(vtoken)) {
    const td = videoTokens.get(vtoken);
    const tokenIp = td.ip;
    const ipMatch = tokenIp === clientIp || tokenIp === clientIp?.replace("::ffff:","") || clientIp === "::ffff:" + tokenIp || tokenIp === "::1" || clientIp === "::1";
    if (td.expires > Date.now() && td.fid === f.id && ipMatch) {
      tokenUser = { username: td.username, role: td.role };
    }
  }
  ...
  if (!u) {
    ...
    if (token) {
      try {
        const pl = jw.verify(token, JWT_SECRET);
        const dbUser = getUser(pl.username);
        if (dbUser) u = dbUser;
      } catch(e) {}
    }
  }
...
// Admin: get watch stats for a file
...
// Admin: get top watched seconds across all files
app.get("/api/admin/watch-stats-all", ownerMW, (req, res) => {
  const result = [];
  for (const [fid, ws] of watchStats.entries()) {
    const f = getFile(fid);
    const fname = f ? (_2x(f.originalName, DATA_KEY) || "unknown") : fid;
    const topSeconds = Object.entries(ws.heatmap)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
